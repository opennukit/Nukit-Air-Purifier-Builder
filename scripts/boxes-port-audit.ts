...
const portedGenerators: PortedGenerator[] = [
  {
    ...
    translated: "src/boxes/generators/airPurifier.ts",
...
const portFiles = [
  ...listFiles(join(repoRoot, "src", "boxes"), (path) => path.endsWith(".ts")),
  join(repoRoot, "src", "assemblyModel.ts"),
  join(repoRoot, "src", "airPurifierPanels.ts"),
  join(repoRoot, "src", "cutGeometry.ts"),
