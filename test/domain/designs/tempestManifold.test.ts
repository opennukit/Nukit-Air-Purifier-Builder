...
describe("Tempest meshes are 2-manifold", () => {
  ...
  test("the wall cord bore carries a drillable 45-degree boss on the inside face", () => {
    ...
    // parts-list item) so builders can drill the hole out for larger connectors.
    // Default model: right wall, cord near the floor corner; the boss face ring
    // reaches bore/2 + 4mm and grows at 45 degrees toward the wall, so the probes
    // below sit inside the meat at the default diameter.
    const model = createTempestModel(defaultTempestSettings);
